...
ax_curve.plot(angles_rad * (180/np.pi), q_curve, color=T['DATA'], linewidth=2.5,
              label='Quantum (P = sin^2 Delta theta)')
ax_curve.plot(angles_rad * (180/np.pi), c_curve, color=T['PINK'], linestyle='--',
              linewidth=2, label='Classical Linear Bound')

# Dynamic points tracking current slider values
point_q, = ax_curve.plot([], [], 'o', color=T['DATA'], markersize=10, label='Current Quantum State')
point_c, = ax_curve.plot([], [], 'o', color=T['WARN'], markersize=10, label='Current Classical State')
...
circle_a = plt.Circle(( -0.75, 0), 0.7, color='#555555', fill=False, linestyle='--')
circle_b = plt.Circle((  0.75, 0), 0.7, color='#555555', fill=False, linestyle='--')
ax_visual.add_patch(circle_a)
ax_visual.add_patch(circle_b)

# Detector polarization axis lines
line_a, = ax_visual.plot([], [], '-', color=T['DATA'], linewidth=3, label=r"Detector A ($\theta$)")
line_b, = ax_visual.plot([], [], '-', color=T['WARN'], linewidth=3, label=r"Detector B ($\phi$)")
ax_visual.legend(loc='lower center')

# Text box to display live calculated values
text_box = ax_visual.text(-1.4, -1.3, "", fontsize=11, fontweight='bold',
                          bbox=dict(facecolor='#333333', alpha=0.9, edgecolor='#666666'))

# ------------------------------------------
# Interactive Sliders Setup
# ------------------------------------------
ax_slider_theta = plt.axes([0.2, 0.12, 0.6, 0.03], facecolor='#2a2a2a')
ax_slider_phi   = plt.axes([0.2, 0.06, 0.6, 0.03], facecolor='#2a2a2a')
